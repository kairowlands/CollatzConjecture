...
int main(){
    int StepsToOne = 0; // Define the amount of steps to reduce each integer to one
    int MaxNumber = 1;  // Define the integer which requires most amount of steps to reduce to one
    ...
    for(int MaxInteger = 1; MaxInteger < 1000; MaxInteger++){
        ...
        while(n != 1){
            if(n % 2 == 1){    // If n is odd
                ...
                n /= 2; // If n is even, then divide by two
            }
            steps++;    // Tells the loop to repeat until the integer is reduced to one
        }
        if(StepsToOne < steps){
            StepsToOne = steps;     // Stores the largest value (which corresponds to the most amount of steps) and assigns its value to StepsToOne
